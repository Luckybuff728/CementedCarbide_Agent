    ...
    <div class="prediction-summary">
      ...
      <div class="metrics-grid">
        <div class="metric-item">
          <span>结合力</span>
          <span>{{ predictionData.adhesion }}</span>
        </div>
        <div class="metric-item">
          <span>氧化温度</span>
          <span>{{ predictionData.oxidation }}°C</span>
        </div>
      </div>
...
const predictionData = computed(() => {
  ...
  // 检查是否为对象类型
  if (!pred || typeof pred !== 'object') return { hardness: 'N/A', adhesion: 'N/A', oxidation: 'N/A', confidence: 0 }
  
  return {
    hardness: pred.hardness ?? 'N/A',
    adhesion: pred.adhesion_strength ?? 'N/A',
    oxidation: pred.oxidation_temperature ?? 'N/A',
