...
const metricsConfig = [
  ...
  { key: 'adhesion_strength', label: '结合力', unit: 'N', color: '#3b82f6' },
  { key: 'oxidation_temperature', label: '氧化温度', unit: '℃', color: '#10b981' },
  { key: 'wear_rate', label: '磨损率', unit: 'mm³/Nm', color: '#f59e0b' },
  { key: 'surface_roughness', label: '表面粗糙度', unit: 'μm', color: '#ef4444' }
]
  ...
  metricsConfig.forEach((metric, index) => {
    ...
    dataSources.forEach((dataSource, dataIndex) => {
      ...
      const formatValue = (val) => {
        if (metric.key === 'oxidation_temperature') return val.toFixed(0)
